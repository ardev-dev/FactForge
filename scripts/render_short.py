...
def render(video_id: str):
    ...
    print("\n[2/2] FFmpeg audio merge (lossless video — preserves original colors)...")
    # Pure stream copy on video — NO re-encode, NO color grading.
    # Footage colors stay exactly as Remotion rendered them.
    # Only audio is encoded to AAC for YouTube compatibility.
    ffmpeg_cmd = [
        "ffmpeg", "-y",
        "-i", str(noaudio_path),
        "-i", str(audio_path),
        "-map", "0:v:0",
        "-map", "1:a:0",
        "-c:v", "copy",          # ← ZERO quality loss; original colors preserved
        "-c:a", "aac",
        "-b:a", "256k",
        "-movflags", "+faststart",
        "-shortest",
